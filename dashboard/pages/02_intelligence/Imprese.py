...
from sources import load_mart_imprese, query_duckdb, _clean_path, _latest_year
...
st.plotly_chart(fig, use_container_width=True, key="imprese_ruolo_pie")

# -- Top imprese per attivita ---------------------------------------------------
st.subheader("Top imprese per attivita totale")
...
st.dataframe(
    ...
    column_config={
        ...
        "totale_attivita": st.column_config.NumberColumn("Totale attivita"),
    },
)
...
fig2 = px.bar(
    ...
    title=f"Top {n_top} imprese per attivita",
    labels={"denominazione": "Impresa", "totale_attivita": "Totale attivita"},
)
fig2.update_layout(yaxis={"categoryorder": "total ascending"}, height=max(300, n_top * 25))
st.plotly_chart(fig2, use_container_width=True, key="imprese_top_attivita")
...
part_p = _clean_path("anac_partecipanti", _latest_year("anac_partecipanti", 2026))
if part_p:
    rti_data = query_duckdb(f"""
        SELECT
            p1.cig,
...
st.markdown("Chi riceve i subappalti: imprese che lavorano per conto di altre.")

sub_p = _clean_path("anac_subappalti", _latest_year("anac_subappalti", 2026))
if sub_p:
    sub_data = query_duckdb(f"""
        SELECT
            denominazione,
    ...
    if not sub_data.empty:
        ...
        st.plotly_chart(fig3, use_container_width=True, key="imprese_subappalti")
...
ruolo_filter = st.selectbox(
    "Filtra per ruolo",
    ["Tutti", "aggiudicatario", "partecipante", "subappaltante", "sconosciuto"],
)
